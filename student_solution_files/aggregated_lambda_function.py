...
def lambda_handler(event, context):
    ...
    table = dynamodb.Table('2401FTDE-HLANDL-portfolio-data-table')
    ...
    # --- Insert your code here ---
    sentiment = comprehend.detect_sentiment(Text=enquiry_text, LanguageCode='en') # ['Sentiment'] # <---Insert code to get the sentiment with AWS comprehend
    # -----------------------------
    ...
    # --- Insert your code here ---
    critical_phrase_list = ['Article', 'article', 'Articles', 'articles', 'blogs', 'posts']
    # Do not change the name of this variable
    email_text = email_response(dec_dict['name'], critical_phrase_list, phrase, sentiment) 

    # Maximum number of consecutive spaces to allow
    max_consecutive_spaces = 1

    # Remove spaces if more than max_consecutive_spaces
    cleaned_string = ''
    ...
    for char in email_text:
        if char == ' ':
            ...
            cleaned_string += char
            consecutive_spaces = 0  # Reset consecutive spaces counter if non-space character is encountered
    
    email_text = cleaned_string
